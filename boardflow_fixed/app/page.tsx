...
async function readResponseBody(res: Response): Promise<Record<string, unknown> | null> {
  try {
  ...
  async function createBoard() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const fallback = `Failed to create board (${res.status})`
        throw new Error(apiError || fallback)
      }
      if (!data?.id) throw new Error('Invalid response from server')
      router.push(`/board/${data.id}`)
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to create board. Try again.'
      setError(message)
      setLoading(false)
    }
  }
